...
public class GuideBoardActivity extends BasicActivity {
    ...
    private ArrayList<BoardInfo> guideBoardList;
    @Override
    ...
    OnPostListener onPostListener = new OnPostListener() { //메인 화면에서 게시물 수정 삭제시 발생되는 리스너
        ...
        public void onModify(int position) {
            myStartActivity(WriteGuidePostActivity.class, postList.get(position));
        }
    };
    ...
    View.OnClickListener onClickListener = v -> {
        switch (v.getId()) {
            case R.id.floatingActionButton:
                myStartActivity(WriteGuidePostActivity.class, apiId); //게시글 작성 화면으로 이동
                break;
        }
    };
    ...
    private void postUpdate() { //게시물 갱신 메소드
        ...
        if (firebaseUser != null) { //사용자가 로그인 되었다면
            if(apiId != null && !apiId.equals("default")){
                BoardInfo boardInfo = new BoardInfo(apiId, apiTitle);
                FirebaseFirestore db = FirebaseFirestore.getInstance();
                db.collection("guide_board_list").document(apiId).set(boardInfo)
                        .addOnSuccessListener(aVoid -> {
                                });
                CollectionReference collectionReference = firebaseFirestore.collection("guide_posts").document(apiId).collection("board");
                collectionReference
    ...
    private void storeUploader(String postId) { //Store 수정
        if (storageDeleteCount == 0) {
            firebaseFirestore.collection("guide_posts").document(postId)
                    .delete()
                    .addOnSuccessListener(aVoid -> {
