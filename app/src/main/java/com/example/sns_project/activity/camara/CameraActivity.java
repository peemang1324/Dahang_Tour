...
public class CameraActivity extends AppCompatActivity {
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
            = reader -> {
                ...
                File mFile = new File(getExternalFilesDir(null),"profile.jpg"); //휴대폰에 이미지 저장
                ...
                try {
                    ...
                    intent.putExtra(INTENT_PATH,mFile.toString()); //file 경로를 memberInitActivity에 넘겨준다
                    setResult(Activity.RESULT_OK, intent);
                    camera2BasicFragment.closeCamera(); //카메라 종료
                    finish(); //카메라 화면 종료
                }
            };
